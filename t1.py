...
def create_mininet_topology(controller_assignments, net):
    ...
    for switch_id, controller_id in controller_assignments.items():
        other_switches = [s_id for s_id in switches.keys() if s_id != switch_id]
        for other_switch_id in other_switches:
            net.addLink(switches[switch_id], switches[other_switch_id])
