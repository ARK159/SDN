...
def main(mininet_topology, file_data):
    ...
    print("Controllers in Mininet topology:", mininet_topology.controllers)

    create_mininet_topology(controller_assignments, mininet_topology)
...
def create_mininet_topology(controller_assignments, net):
    controller_names = set(controller_assignments.values())
    controllers = {}  # Dictionary to store controller instances

    # Create controllers
    for controller_name in controller_names:
        controllers[controller_name] = net.addController(controller_name, controller=RemoteController, ip='127.0.0.1', port=6633)
    ...
    controller_list = list(controller_names)
    for i in range(len(controller_list)):
        for j in range(i + 1, len(controller_list)):
            net.addLink(controllers[controller_list[i]], controllers[controller_list[j]])

    # Connect switches to each other (added part)
    switch_list = list(switches.keys())
    for i in range(len(switch_list)):
        for j in range(i + 1, len(switch_list)):
            net.addLink(switches[switch_list[i]], switches[switch_list[j]])
